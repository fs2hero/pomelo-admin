...
MqttClient.prototype.connect = function(host, port, cb) {
	...
	this.socket.on('connack', function() {
		...
		logger.debug('mqttClient connack');
	...
	this.socket.on('close', function() {
		logger.error('mqtt socket is close, remote server host: %s, port: %s, id: %j ', host, port, self.id);
		self.onSocketClose();
	});

	this.socket.on('error', function(err) {
		logger.error('mqtt socket is error, remote server host: %s, port: %s, id: %j', host, port, self.id);
	...
	this.socket.on('pingresp', function() {
		...
		logger.debug('client recv pingresp');
	});

	this.socket.on('disconnect', function() {
		logger.error('mqtt socket is disconnect, remote server host: %s, port: %s, id: %j ', host, port, self.id);
		self.emit('disconnect', self.id);
		self.onSocketClose();
	});

	this.socket.on('timeout', function(reconnectFlag) {
		logger.error('timeout ',reconnectFlag);
...
MqttClient.prototype.onSocketClose = function() {
	logger.debug('onSocketClose ' + this.closed);
	if (this.closed) {
		return;
	}
...
MqttClient.prototype.checkKeepAlive = function() {
	...
	// 			logger.error('mqtt rpc client checkKeepAlive error timeout for %d', KEEP_ALIVE_TIMEOUT,' lastPing ',this.lastPing,' lastPong ',this.lastPong);
	// 			this.close();
	// 		}
	// 	} else {
	// 		this.lastPing = Date.now();
	// 		this.socket.pingreq();
	// 		logger.debug('client send pingreq 1');
	// 	}
	// } else {
	// 	this.lastPing = Date.now();
	// 	this.socket.pingreq();
	// 	logger.debug('client send pingreq 2');
	// }
	var now = Date.now();
	var KEEP_ALIVE_TIMEOUT = this.keepalive * 2;
	if (this.lastPing > 0) {
		if (this.lastPong < this.lastPing) {
			if (now - this.lastPong > KEEP_ALIVE_TIMEOUT) {
				...
				this.lastPing = Date.now();
				this.socket.pingreq();
				logger.debug('client send pingreq 3, ',this.lastPing);
			}
		} else {
			this.lastPing = Date.now();
			this.socket.pingreq();
			logger.debug('client send pingreq 1, ',this.lastPing);
		}
	} else {
		this.lastPing = Date.now();
		this.socket.pingreq();
		logger.debug('client send pingreq 2, ',this.lastPing);
	}
}
